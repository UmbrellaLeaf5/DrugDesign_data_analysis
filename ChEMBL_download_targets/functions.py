...
def ExpandedFromDictionariesTargetsDF(data: pd.DataFrame) -> pd.DataFrame:
  ...
  def ExtractedValuesFromColumn(
    df: pd.DataFrame, column_name: str, key: str
  ) -> pd.Series:
    """
  ...
  target_components_data = pd.DataFrame(data["target_components"].values.tolist())
...
def AddedIC50andKiToTargetsDF(data: pd.DataFrame) -> pd.DataFrame:
  ...
  v_logger.info(
    "Adding 'IC50' and 'Ki' columns to pandas.DataFrame...", LogMode.VERBOSELY
  )

  # добавляем столбец 'IC50', подсчитывая активности.
  data["IC50"] = data["target_chembl_id"].apply(CountTargetActivitiesByIC50)
  ...
  if targets_config["download_activities"]:
    ...
    try:
      ...
      data = data[(data["IC50_new"].notna()) & (data["Ki_new"].notna())]
...
def DownloadTargetsFromIdList():
  ...
  # формируем имя файла.
  file_name: str = (
    f"{targets_config['results_folder_name']}/{targets_config['results_file_name']}.csv"
  )
