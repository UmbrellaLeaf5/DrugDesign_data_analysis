...
*   **Общие настройки:**
    *   `testing_flag`: логический флаг, который, когда установлен в `true`, включает режим тестирования (ограничивает количество загружаемых данных для ускорения тестирования).
    *   `skip_downloaded`: логический флаг, указывающий, следует ли пропускать загрузку, если файлы уже существуют (полезно для повторного запуска скрипта без перезагрузки данных).
    *   `csv_separator`: строка, определяющая разделитель полей в CSV-файлах.
    *   `keyboard_end_message`: сообщение, которое выводится при завершении загрузки по нажатию клавиш (`Ctrl+C`/`Ctrl+Z`).

*   **ChEMBL_download_activities:** конфигурация для загрузки данных об активностях (`IC50`, `Ki`) из `ChEMBL`.
    *   `logger_label`: метка, используемая для сообщений журнала, связанных с этой задачей (для идентификации в логах).
...
*   **ChEMBL_download_cell_lines:** конфигурация для загрузки данных о клеточных линиях из `ChEMBL`.
    ...
    *   `raw_csv_g_drive_id`: идентификатор Google.Drive архива, в котором лежат неочищенные файлы с необходимыми активностями (необходим, так как активности к клеточным линиям через интерфейс `chembl_webresource_client` или API ChEMBL - не вышло).
...
*   **ChEMBL_download_compounds:** конфигурация для загрузки данных о соединениях из `ChEMBL`.
    ...
    *   `molfiles_folder_name`: имя папки для хранения mol- и sdf-файлов соединений.
    ...
    *   `delete_after_combining`: логический флаг, указывающий, следует ли удалять оставшиеся данные после объединения.
...
*   **ChEMBL_download_targets:** конфигурация для загрузки данных о мишенях из `ChEMBL`.
    ...
    *   `download_activities`: логический флаг, указывающий, следует ли загружать данные об активностях для мишеней.
...
*   **PubChem_download_toxicity:** конфигурация для загрузки данных о токсичности из `PubChem`.
    ...
    *   `results_file_name`: имя файла для сохранения данных о токсичности.
    *   `combined_file_name`: имя файла для сохранения объединенных данных о токсичности.
    *   `need_combining`: логический флаг, указывающий, нужно ли объединять данные.
    *   `delete_after_combining`: логический флаг, указывающий, следует ли удалять оставшиеся данные после объединения.
    *   `filtering`:  словарь, содержащий параметры фильтрации данных о токсичности.
        *   `organism`: список организмов для фильтрации.
        *   `route`: список способов введения для фильтрации.
        *   `dose`:  строка, определяющая единицы измерения дозы.
    *   `sleep_time`: число с плавающей точкой, представляющее время ожидания между запросами (в секундах, для предотвращения блокировки со стороны PubChem).
    ...
    *   `limit`: целое число, представляющее лимит для кол-ва данных за 1 запрос.

*   **Utils:**  конфигурация для различных утилит, используемых в проекте.
    *   `CombineCSVInFolder`: конфигурация для утилиты объединения CSV файлов.
        *   `logger_label`: метка, используемая для сообщений журнала, связанных с этой задачей.
        *   `logger_color`: цветовой код для вывода журнала.
    *   `VerboseLogger`: конфигурация для детального логирования.
        * `verbose_print`:  логический флаг, указывающий, включен ли подробный вывод в консоль.
        * `message_ljust`:  ширина левого выравнивания для сообщений лога.
        * `exceptions_file`: имя файла для записи исключений.
        * `output_to_exceptions_file`: логический флаг, указывающий, следует ли выводить весь вывод только в файл для записи исключений.
    *   `ReTry`: конфигурация для повторных попыток выполнения операций.
        * `attempts_amount`: количество попыток по умолчанию.
        * `sleep_time`: время ожидания между попытками (в секундах) по умолчанию.
