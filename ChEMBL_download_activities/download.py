...
def GetCellLineChEMBLActivitiesFromCSV(cell_lines_data: pd.DataFrame):
    ...
    for cell_id in cell_lines_data['cell_chembl_id']:
        ...
        v_logger.info(
            f"Getting activities connected with {cell_id}...", LogMode.VERBOSELY)
        ...
        cell_lines_data.loc[
            cell_lines_data["cell_chembl_id"] == cell_id, "IC50_new"] =\
            len(data_frame_ic50)

        cell_lines_data.loc[
            cell_lines_data["cell_chembl_id"] == cell_id, "GI50_new"] =\
            len(data_frame_gi50)
